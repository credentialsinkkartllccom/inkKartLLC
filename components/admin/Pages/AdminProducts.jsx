...
const AdminProducts = () => {
    ...
    // Helper to detect if selected category is Ink & Toner
    const selectedCategoryObj = categories?.find(c => c._id === formData.category);
    const isInkTonerSelected = !!(selectedCategoryObj?.name &&
        selectedCategoryObj.name.toLowerCase().includes('ink') &&
        selectedCategoryObj.name.toLowerCase().includes('toner'));
    ...
    const handleInputChange = (e) => {
        ...
        if (type === 'checkbox' && ARRAY_FIELDS.includes(name)) {
            ...
            if (name === 'category') {
                ...
                const isInkToner = catName.toLowerCase().includes('ink') && catName.toLowerCase().includes('toner');
    ...
    const handleEdit = (product) => {
        ...
        const isInkTonerProduct = categoryName.toLowerCase().includes('ink') && categoryName.toLowerCase().includes('toner');
    ...
    const handleBulkUpload = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_URL || '/api'}/products/bulk-upload`, formData, config);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
            ...
            {isFormOpen && (
                <div className="bg-[var(--bg)] rounded-3xl border border-slate-200 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-6 duration-300">
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-12">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* Printer-specific fields — hide for Ink & Toner */}
                            {!isInkTonerSelected && (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-2">
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            {TECHNOLOGY_OPTIONS.map(opt => (
                                                <label key={opt} className="flex items-center gap-2 text-sm font-semibold text-slate-700 cursor-pointer">
                                                    <input type="checkbox" name="technology" value={opt} checked={formData.technology.includes(opt)} onChange={handleInputChange} className="accent-blue-600" />
                                                    {opt}
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            {USAGE_CATEGORY_OPTIONS.map(opt => (
                                                <label key={opt} className="flex items-center gap-2 text-sm font-semibold text-slate-700 cursor-pointer">
                                                    <input type="checkbox" name="usageCategory" value={opt} checked={formData.usageCategory.includes(opt)} onChange={handleInputChange} className="accent-blue-600" />
                                                    {opt}
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            {ALL_IN_ONE_OPTIONS.map(opt => (
                                                <label key={opt} className="flex items-center gap-2 text-sm font-semibold text-slate-700 cursor-pointer">
                                                    <input type="checkbox" name="allInOneType" value={opt} checked={formData.allInOneType.includes(opt)} onChange={handleInputChange} className="accent-blue-600" />
                                                    {opt}
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {MAIN_FUNCTION_OPTIONS.map(opt => (
                                                <label key={opt} className="flex items-center gap-2 text-sm font-semibold text-slate-700 cursor-pointer">
                                                    <input type="checkbox" name="mainFunction" value={opt} checked={formData.mainFunction.includes(opt)} onChange={handleInputChange} className="accent-blue-600" />
                                                    {opt}
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <select name="wireless" value={formData.wireless} onChange={handleInputChange} className="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none font-bold text-slate-800">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex bg-slate-100 rounded-lg p-1">
                                            <button
                                                ...
                                                className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${specType === 'text' ? 'bg-[var(--bg)] text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                            >
                                                Text Editor
                                            </button>
                                            <button
                                                ...
                                                className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${specType === 'table' ? 'bg-[var(--bg)] text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                            >
                                                Table Builder
                                            </button>
                                        </div>
                                            ...
                                            {specRows.map((row) => (
                                                <div key={row.id} className="flex gap-4 items-start">
                                                    <input
                                                        ...
                                                        className="flex-1 px-4 py-2 bg-[var(--bg)] border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none"
                                                    />
                                                    <input
                                                        type="text"
                                ...
                                {formData.reviews.map((review, index) => (
                                    <div key={index} className="p-6 bg-slate-50 rounded-3xl border border-slate-200 relative group animate-in slide-in-from-right-4 duration-300">
                                        <button
                                            ...
                                            className="absolute -top-2 -right-2 p-2 bg-[var(--bg)] text-red-500 rounded-full shadow-lg hover:bg-red-500 hover:text-white transition-all border border-slate-100"
                                        >
                                            <X size={16} />
                                        </button>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                            <div className="space-y-1">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full px-4 py-2 bg-[var(--bg)] border border-slate-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-pink-500/20"
                                            ...
                                            <div className="space-y-1 md:col-span-2 lg:col-span-1">
                                                ...
                                                <textarea
                                                    ...
                                                    className="w-full px-4 py-2 bg-[var(--bg)] border border-slate-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-pink-500/20 resize-none"
                                                    rows="1"
                        ...
                        {/* Submit Button */}
                        <div className="pt-10 border-t border-slate-200 flex justify-end sticky bottom-0 bg-[var(--bg)]/95 backdrop-blur-sm px-4 py-6 rounded-b-3xl z-10 shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.1)]">
                            <button
                                type="submit"
            ...
            {/* Product List */}
            <div className="bg-[var(--bg)] border border-slate-200 rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50">
                <div className="bg-slate-50/50 border-b border-slate-100 px-8 py-5 flex items-center justify-between">
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="pl-12 pr-6 py-3 bg-[var(--bg)] border border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 w-80 font-bold transition-all"
                        />
                    </div>
                </div>
                                ...
                                <tr key={p._id} className="hover:bg-blue-50/30 transition-all group">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-6">
                                            <div className="w-20 h-20 bg-[var(--bg)] rounded-2xl overflow-hidden border border-slate-100 shadow-sm flex-shrink-0 flex items-center justify-center p-2 group-hover:scale-105 transition-transform rotate-1 group-hover:rotate-0">
                                                <img
                                                    src={p.images?.[0] ? (p.images[0].startsWith('http') ? p.images[0] : p.images[0]) : '/printer.png'}
                                                    alt=""
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex items-center justify-end gap-3">
                                            <button onClick={() => handleEdit(p)} className="p-4 bg-[var(--bg)] hover:bg-slate-900 text-slate-400 hover:text-white rounded-2xl shadow-xl shadow-slate-100 border border-slate-100 transition-all active:scale-95">
                                                <Edit size={22} />
                                            </button>
                                            <button onClick={() => handleDelete(p._id)} className="p-4 bg-[var(--bg)] hover:bg-red-600 text-slate-400 hover:text-white rounded-2xl shadow-xl shadow-slate-100 border border-slate-100 transition-all active:scale-95">
                                                <Trash2 size={22} />
                                            </button>
                                        </div>
                                    </td>
