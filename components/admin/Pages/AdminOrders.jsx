...
const AdminOrders = () => {
    ...
    const fetchOrders = async (pageNum = 1, append = false, search = '') => {
        try {
            setLoading(true);
            const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || '/api'}/orders?page=${pageNum}&search=${search}`, {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
            ...
            {/* Orders Table */}
            <div className="bg-[var(--bg)] border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            ...
            {isUpdateModalOpen && selectedOrder && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[var(--bg)] w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 m-4">
                        ...
                        <form onSubmit={handleUpdateSubmit} className="p-6 space-y-4">
                            <div>
                                ...
                                <select
                                    className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-[var(--bg)] font-medium text-slate-700"
            ...
            {isItemsModalOpen && selectedOrder && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[var(--bg)] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 m-4">
                        <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                            <div>
                            ...
                            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                                {selectedOrder.orderItems.map((item, index) => (
                                    <div key={index} className="flex gap-4 items-center p-3 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
                                        <div className="w-16 h-16 bg-[var(--bg)] border rounded-lg flex items-center justify-center overflow-hidden shrink-0">
            ...
            {isPaymentModalOpen && selectedOrder && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[var(--bg)] w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 m-4">
                        <div className={`px-6 py-6 text-white text-center ${selectedOrder.isPaid ? 'bg-gradient-to-r from-emerald-500 to-emerald-600' : 'bg-gradient-to-r from-slate-500 to-slate-600'}`}>
                            <div className="w-12 h-12 bg-[var(--bg)]/20 rounded-full flex items-center justify-center mx-auto mb-3 backdrop-blur-sm">
