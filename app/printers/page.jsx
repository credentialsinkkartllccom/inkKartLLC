...
const PrintersContent = () => {
    ...
    // Fetch larger batch of products to manually filter ensuring data is found across categories
    useEffect(() => {
        setAllProducts([]);
        setCurrPage(1);
    ...
    const filteredAndSorted = useMemo(() => {
        let filtered = allProducts.filter(p => {
            ...
            // 1. Core Category Filter (Printers only, excluding consumables)
            // Check category name, title, description, and technology fields
            ...
            const isConsumableTitle = (fullName.includes('ink') || fullName.includes('toner') || fullName.includes('cartridge')) && !isPrinterTitle;
            const hasTechnology = p.technology && p.technology.length > 0;

            const isLikelyPrinter = (isPrinterCat && !isConsumableCat) || isPrinterTitle || hasTechnology;
            
            if (!isLikelyPrinter) return false;

            // 2. Sidebar Search
            if (filters.search && !fullName.includes(filters.search.toLowerCase())) return false;

            // 3. Brand Filter (Property + Title check)
            ...
            // 4. Category Filter (Manual selection from sidebar)
            if (filters.category.length > 0 && !filters.category.map(c => c.toLowerCase()).includes(catName)) return false;

            // 5. Printer Type Filter (Looking for keywords in description/title/category)
            if (filters.printerType.length > 0) {
                const typeMatches = filters.printerType.some(type => {
                    const t = type.toLowerCase();
                    if (t.includes('laser')) return fullName.includes('laser') || catName.includes('laser');
                    if (t.includes('inkjet')) return fullName.includes('inkjet') || catName.includes('inkjet');
                    return false;
                });
            ...
            if (filters.useCase.length > 0) {
                const desc = (p.description || '').toLowerCase();
                const useCaseMatches = filters.useCase.some(uc => {
                    const u = uc.toLowerCase();
                    const words = u.split(' & ').join(' ').split(' ');
                    return words.some(w => w.length > 2 && (fullName.includes(w) || desc.includes(w)));
                });
        ...
        <div className="category-view-elite">
            <header className="cat-hero-dark reveal" style={{
                ...
                <div className="cat-hero-container">
                    <span className="cat-pretitle" style={{ color: '#ffffff' }}>Advanced Hardware</span>
                    <h1 className="cat-main-title" style={{ color: '#ffffff' }}>Printers <span style={{ color: '#ffffff' }}>& Systems.</span></h1>
                    <p className="cat-meta-desc" style={{ color: '#f8fafc' }}>
                        Engineering excellence for the modern enterprise. Explore our collection of 
                        high-performance laser, inkjet, and multifunction architectures designed 
                        for maximum throughput and precision.
                    </p>
                </div>
                            ...
                            !loading && (
                                <div className="no-results-premium reveal">
                                    <div className="no-results-content">
                                        ...
                                        <p className="no-results-desc">No assets match your current configuration. Consider recalibrating filters.</p>
            ...
            <style>{`
                ...
                .cat-hero-bg-accent { position: absolute; top: -50%; right: -10%; width: 60%; height: 200%; background: linear-gradient(45deg, rgba(217, 165, 32, 0.05), transparent); transform: rotate(15deg); }
